...
export default function QrCodeLibraryPage({ library, role, icons, onCreatePage, onEditPage, user, workspaceId }: Props) {
  ...
  return <>
    ...
    {library.status === "ready" && library.codes.length > 0 && (codes.length === 0 ? <section className="deck-library-status"><h2>No matching QR codes</h2><p>Try a different search.</p></section> : <section className="slide-card-grid">{codes.map((code) => <article className="slide-card" key={code.id}><span className="deck-status">{code.kind}</span><h2>{code.name}</h2><p>{code.content}</p><QrCodeCanvas content={qrPayload(code)} color={code.color} version={code.version} logoScale={code.logoScale} iconImage={icons.find((icon) => icon.id === code.iconId)?.imageDataUrl} /><small>{code.scanCount ?? 0} tracked scans</small>{canEdit && <><button type="button" onClick={() => onEditPage(code.id)}>Edit QR code</button><button className="workspace-text-button" type="button" onClick={() => { if (window.confirm(`Archive “${code.name}”?`)) void archiveResource(user, workspaceId, "qr-codes", code.id).then(() => window.location.reload()); }}>Archive</button></>}</article>)}</section>)}
  </>;
}
