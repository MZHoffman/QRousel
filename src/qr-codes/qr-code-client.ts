...
export async function updateQrCode(user: AuthenticatedUser, workspaceId: string, qrCodeId: string, input: { name: string; kind: QrCodeKind; value: string; color: string; version: number; expectedRevision: number }): Promise<{ kind: "updated"; qrCode: QrCodeSummary } | { kind: "conflict"; qrCode: QrCodeSummary }> {
