...
function MembersPanel({ user, workspaceId, actorRole }: { user: User; workspaceId: string; actorRole: WorkspaceRole }) {
  const [members, setMembers] = useState<WorkspaceMember[]>([]); const [invitations, setInvitations] = useState<WorkspaceInvitation[]>([]); const [error, setError] = useState(""); const [invitationError, setInvitationError] = useState(""); const [busyUid, setBusyUid] = useState("");
  const roles = manageableRoles(actorRole); const canManage = roles.length > 0;
  useEffect(() => { let active = true; void requestMembers(user, workspaceId).then((next) => { if (active) { setMembers(next); setError(""); } }, (reason: unknown) => { if (active) setError(reason instanceof Error ? reason.message : "QRousel could not load workspace members."); }); if (canManage) void requestInvitations(user, workspaceId).then((next) => { if (active) { setInvitations(next); setInvitationError(""); } }, (reason: unknown) => { if (active) setInvitationError(reason instanceof Error ? reason.message : "QRousel could not load invitations."); }); return () => { active = false; }; }, [canManage, user, workspaceId]);
  ...
  return <div className="members-page"><section className="workspace-member-list">{members.map((member) => { const canEdit = roles.includes(member.role as Exclude<WorkspaceRole, "founder">) && member.uid !== user.uid; return <article className="workspace-list-card member-card" key={member.uid}><div className="workspace-member-avatar" aria-hidden="true">{(member.displayName?.[0] ?? member.email?.[0] ?? "U").toUpperCase()}</div><div><strong>{member.displayName ?? member.email ?? "Workspace member"}</strong><span>{member.email ?? "No email available"}</span></div>{canEdit ? <div className="member-card-actions"><select value={member.role} disabled={busyUid === member.uid} onChange={(event) => void changeRole(member, event.target.value as Exclude<WorkspaceRole, "founder">)}>{roles.map((role) => <option key={role}>{role}</option>)}</select><button type="button" disabled={busyUid === member.uid} onClick={() => void remove(member)}>Remove</button>{actorRole === "founder" && <button type="button" disabled={busyUid === member.uid} onClick={() => void handoff(member)}>Make founder</button>}</div> : <span className="workspace-role">{roleLabel(member.role)}</span>}</article>; })}</section>{error && <p className="auth-error" role="alert">{error}</p>}{canManage && <InvitePanel user={user} workspaceId={workspaceId} roles={roles} onCreated={(invitation) => setInvitations((items) => [invitation, ...items])} loadError={invitationError} />}{canManage && invitations.length > 0 && <section className="invitation-list"><h2>Invite links</h2>{invitations.map((invitation) => <article key={invitation.token}><div><strong>{invitation.email ?? "Shareable invite link"}</strong><span>{roleLabel(invitation.role)} access</span></div><span>{invitation.status === "active" ? "Active" : "Used"}</span></article>)}</section>}</div>;
}
function InvitePanel({ user, workspaceId, roles, onCreated, loadError }: { user: User; workspaceId: string; roles: Exclude<WorkspaceRole, "founder">[]; onCreated: (invitation: WorkspaceInvitation) => void; loadError: string }) {
  const defaultRole = roles.includes("editor") ? "editor" : roles[0];
  const [emailRole, setEmailRole] = useState<Exclude<WorkspaceRole, "founder">>(defaultRole);
  const [linkRole, setLinkRole] = useState<Exclude<WorkspaceRole, "founder">>(defaultRole);
  const [link, setLink] = useState(""); const [email, setEmail] = useState(""); const [error, setError] = useState(""); const [delivery, setDelivery] = useState(""); const [sendingEmail, setSendingEmail] = useState(false); const [creatingLink, setCreatingLink] = useState(false);
  async function create(role: Exclude<WorkspaceRole, "founder">, inviteEmail?: string) {
    setError(""); setDelivery("");
    try {
      const invitation = await createInvitation(user, workspaceId, role, inviteEmail);
      const inviteLink = `${window.location.origin}/app?invite=${encodeURIComponent(invitation.token)}`;
      setLink(inviteLink);
      onCreated({ token: invitation.token, role, status: "active", email: inviteEmail?.trim() || null });
      if (invitation.emailDelivery === "sent") setDelivery("Invitation email sent.");
      else if (inviteEmail?.trim()) setDelivery("Email delivery is not configured. Copy and share this one-use link instead.");
    } catch (reason) { setError(reason instanceof Error ? reason.message : "QRousel could not create an invitation."); }
  }
  async function sendEmailInvitation() { const recipient = email.trim(); if (!recipient || sendingEmail) return; setSendingEmail(true); try { await create(emailRole, recipient); } finally { setSendingEmail(false); } }
  async function createInviteLink() { if (creatingLink) return; setCreatingLink(true); try { await create(linkRole); } finally { setCreatingLink(false); } }
  return <section className="invite-panel"><header><h2>Invite members</h2><p>Choose a direct email invitation or create a private, one-use link to share yourself.</p></header><form className="invite-email-section" onSubmit={(event) => { event.preventDefault(); void sendEmailInvitation(); }}><div><h3>Send an email invitation</h3><p>We’ll email this person a one-use link.</p></div><label><span>Email address</span><input type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="name@example.com" required /></label><label><span>Access</span><select value={emailRole} onChange={(event) => setEmailRole(event.target.value as Exclude<WorkspaceRole, "founder">)}>{roles.map((item) => <option key={item}>{item}</option>)}</select></label><button type="submit" disabled={sendingEmail}>{sendingEmail ? "Sending…" : "Send email invitation"}</button></form><div className="invite-divider" aria-hidden="true"><span>or</span></div><section className="invite-link-section" aria-labelledby="invite-link-heading"><div><h3 id="invite-link-heading">Create a one-use invite link</h3><p>Share it in any channel. It works once, then expires.</p></div><label><span>Access</span><select aria-label="Access level for invite link" value={linkRole} onChange={(event) => setLinkRole(event.target.value as Exclude<WorkspaceRole, "founder">)}>{roles.map((item) => <option key={item}>{item}</option>)}</select></label><button type="button" onClick={() => void createInviteLink()} disabled={creatingLink}>{creatingLink ? "Creating…" : "Create invite link"}</button></section>{delivery && <p className="invite-panel-note">{delivery}</p>}{link && <label className="invite-link"><span>One-use invite link</span><input readOnly value={link} onFocus={(event) => event.currentTarget.select()} /></label>}{(loadError || error) && <p className="auth-error" role="alert">{error || loadError}</p>}</section>;
}
