...
function InvitePanel({ user, workspaceId, roles, onCreated, loadError }: { user: User; workspaceId: string; roles: Exclude<WorkspaceRole, "founder">[]; onCreated: (invitation: WorkspaceInvitation) => void; loadError: string }) {
  ...
  return <section className="invite-panel"><div><h2>Invite a member</h2><p>Share a one-use link, with email delivery if you enter an address.</p></div><div className="invite-panel-form"><label><span>Email <em>optional</em></span><input type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="name@example.com" /></label><label><span>Access</span><select value={role} onChange={(event) => setRole(event.target.value as Exclude<WorkspaceRole, "founder">)}>{roles.map((item) => <option key={item}>{item}</option>)}</select></label><button type="button" onClick={() => void create()}>Create invite link</button></div>{delivery && <p className="invite-panel-note">{delivery}</p>}{link && <label className="invite-link"><span>One-use invite link</span><input readOnly value={link} onFocus={(event) => event.currentTarget.select()} /></label>}{(loadError || error) && <p className="auth-error" role="alert">{error || loadError}</p>}</section>;
}
