...
function InvitePanel({ user, workspaceId, roles, onCreated, loadError }: { user: User; workspaceId: string; roles: Exclude<WorkspaceRole, "founder">[]; onCreated: (invitation: WorkspaceInvitation) => void; loadError: string }) {
  ...
  async function create() { setError(""); setDelivery(""); try { const invitation = await createInvitation(user, workspaceId, role, email); setLink(`${window.location.origin}/app?invite=${encodeURIComponent(invitation.token)}`); onCreated({ token: invitation.token, role, status: "active", email: email.trim() || null }); if (invitation.emailDelivery === "sent") setDelivery("Invitation email sent."); else if (email.trim()) setDelivery("Email was not sent. Copy and share the link instead."); } catch (reason) { setError(reason instanceof Error ? reason.message : "QRousel could not create an invitation."); } }
  const isEmailInvite = email.trim().length > 0;
  return <section className="invite-panel"><div><h2>Invite a member</h2><p>{isEmailInvite ? "We’ll send a one-use invitation to this address." : "Create a one-use link to share by any channel."}</p></div><div className="invite-panel-form"><label><span>Email</span><input type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="name@example.com" /></label><label><span>Access</span><select value={role} onChange={(event) => setRole(event.target.value as Exclude<WorkspaceRole, "founder">)}>{roles.map((item) => <option key={item}>{item}</option>)}</select></label><button type="button" onClick={() => void create()}>{isEmailInvite ? "Send invite email" : "Create invite link"}</button></div>{delivery && <p className="invite-panel-note">{delivery}</p>}{link && <label className="invite-link"><span>One-use invite link</span><input readOnly value={link} onFocus={(event) => event.currentTarget.select()} /></label>}{(loadError || error) && <p className="auth-error" role="alert">{error || loadError}</p>}</section>;
}
