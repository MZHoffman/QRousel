...
function InvitePanel({ user, workspaceId, roles, onCreated }: { user: User; workspaceId: string; roles: Exclude<WorkspaceRole, "founder">[]; onCreated: (invitation: WorkspaceInvitation) => void }) {
  ...
  const [link, setLink] = useState(""); const [email, setEmail] = useState(""); const [error, setError] = useState("");
  async function create() { setError(""); try { const token = await createInvitation(user, workspaceId, role, email); setLink(`${window.location.origin}/app?invite=${encodeURIComponent(token)}`); onCreated({ token, role, status: "active" }); } catch (reason) { setError(reason instanceof Error ? reason.message : "QRousel could not create an invitation."); } }
  return <section className="workspace-list-card invite-panel"><div><strong>Invite a member</strong><span>Email delivery is optional; every invitation also has a single-use link.</span></div><input type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="person@example.com (optional)" /><select value={role} onChange={(event) => setRole(event.target.value as Exclude<WorkspaceRole, "founder">)}>{roles.map((item) => <option key={item}>{item}</option>)}</select><button type="button" onClick={() => void create()}>Create invitation</button>{link && <label><span>Single-use link</span><input readOnly value={link} onFocus={(event) => event.currentTarget.select()} /></label>}{error && <p className="auth-error">{error}</p>}</section>;
}
